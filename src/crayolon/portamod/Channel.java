...
public class Channel {
	...
	//public static int transposer;
	public int transposer;
	
	private Module module;
	private Instrument instrument;
	private Sample sample;
	public int[] global_volume, current_note;
	...
	private int volume, panning, fine_tune, period, porta_period;
	public int key_add;
	...
	public int effect_tick, trigger_tick, fade_out_volume, random_seed;
	...
	public int chanvolfinal, leftgain, rightgain;
	public int mono = 0;
	...
	public Channel( Module mod, int sampling_rate, int[] global_vol, int mon ) {
		mono = mon;
		module = mod;
	...
	public void set_volume( int vol ) {
		...
		if( vol > chanvol_override ) {
			vol = chanvol_override;
		}
		vol = (int)map(vol, 0, 64, 0, globvol_override);
		volume = vol;
	}
	...
	//syphus' edit - this gives me data for the entire current pattern, not just the current row
	public void pattern_notes( int key, int inst_idx, int volume_column, int effect, int effect_param, int currentrow, int numrows ) {
		current_pattern_notes = new int[numrows][5];
		current_pattern_notes[currentrow][ 0 ] = key;
		current_pattern_notes[currentrow][ 1 ] = inst_idx;
		current_pattern_notes[currentrow][ 2 ] = volume_column;
		current_pattern_notes[currentrow][ 3 ] = effect;
		current_pattern_notes[currentrow][ 4 ] = effect_param;
	}
	...
	private void update_envelopes() {
		...
		if( instrument.panning_envelope_active && mono == 0) {
	...
	private int key_to_period( int key ) {
		...
		octave = ( key + transposer << IBXM.FP_SHIFT ) / 12 + sample.transpose;
		if( linear_periods ) {
	...
	public void calculate_amplitude() {
		...
		if( instrument.volume_envelope_active ) {
			...
			if( key_on ) {
				envelope_volume = chanvol_override;
			}
		}
		...
		if( tremolo_volume > chanvol_override ) {
			tremolo_volume = chanvol_override;
		}
	...
	public int calculate_fakevol() {
		int envelope_volume, tremolo_volume, amplitude, fakevol;
		int envelope_panning, mixer_panning, panning_range;
		Envelope envelope;
		envelope_volume = 0;
		if( instrument.volume_envelope_active ) {
