...
public class ProTracker {
	...
	public static Module load_mod( byte[] header_1084_bytes, DataInput data_input ) throws IOException {
		...
		for( channel_idx = 0; channel_idx < num_channels; channel_idx++ ) {
			panning = 120; // 128 for centre, 64 for 25% left, 0 for 100% left
			if( ( channel_idx & 0x03 ) == 0x01 || ( channel_idx & 0x03 ) == 0x02 ) {
				panning = 136; // 128 for centre, 196 for 75% right, 256 for 100% left
			}
	...
	public static Pattern read_mod_pattern( DataInput data_input, int num_channels ) throws IOException {
	...
	public static Instrument read_mod_instrument( byte[] mod_header, int idx, DataInput data_input ) throws IOException  {
