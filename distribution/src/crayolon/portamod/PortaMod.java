...
public class PortaMod {
	...
	public void setGlobvol(int vol){
		if (vol >= 0 && vol <= 64) {
			...
			//globvol = vol;
			globvol = (int)p.map(vol, 0, 64, 0, voloverride);
			player.ibxm.global_volume[0] = globvol;
		} else {
	...
	/**
	 * setGlobvolOverride(int)
	 */
	public void setGlobvolOverride(int override) {
		if (override >= 0 && override <= 64) {
			voloverride = override;
		} else {
			System.out.println("Global volume override must be between 0 and 64");
		}
	}
	...
	public int doModLoad(String tune, boolean autostart, int startVol) {
		...
		InputStream file_input_stream = p.createInput(tune);
			...
			player = new Player(interpolation, mono);
	...
	public void delayTap1(int channel, int volume) {
		...
		try {
			InputStream file_input_stream = p.createInput(modpath);
				delayplayerA = new Player(interpolation, mono);
	...
	*/
	...
	public boolean headerCheck (String tune){
		...
		if (p.createInput(tune) == null) {
			System.out.println("PortaMod says: couldn't find the file at " + tune + " when performing a header check. Please check the filename and path.");
		} else {
			InputStream file_input_stream = p.createInput(tune);
